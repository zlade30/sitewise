...
import { CalendarIcon, FlagIcon, NavLogoFullIcon, NavLogoIcon, ProjectIcon, RoundedVideoIcon, SidebarIcon } from '@/public/icons';
        ...
        <div>
            ...
            >
                <section className="w-full flex flex-col items-center gap-[24px] px-[16px]">
                    {isMinimize ? <NavLogoIcon className="w-[50px] h-[50px]" /> : <NavLogoFullIcon className="w-[200px] h-[100px]" />}
                    {isMinimize ? (
                        <ButtonRounded icon={<ProjectIcon className="w-[18px] h-[18px] text-primary" />} />
                    ) : (
                        <div
                            className="
                ...
                >
                    ...
                    {isMinimize ? <ButtonRounded icon={<CalendarIcon className="w-[18px] h-[18px] text-primary" />} /> : <Calendar />}
                    ...
                    {isMinimize ? (
                        <>
                            {[1, 2, 3].map((item) => (
                                <ButtonRounded key={item} icon={<RoundedVideoIcon className="w-[18px] h-[18px] text-primary" />} />
                            ))}
                        </>
                    ) : (
                        <>
                            {[1, 2, 3].map((item) => (
                                <div key={item} className="w-full rounded-[8px] border border-line p-2 gap-1 flex flex-col">
                                    <p className="text-[12px] font-medium">Up Next:</p>
                                    <div className="flex items-center gap-2 font-medium text-primary">
                                        <RoundedVideoIcon className="w-[18px] h-[18px]" />
                                        <p>10:00 AM</p>
                                    </div>
                                    <p className="text-primary">123FDJD (Christian Bale)</p>
                                    <Button value="Join Meeting & Start Visit" icon={<FlagIcon />} />
                                </div>
                            ))}
                        </>
                    )}
                ...
                <section className="w-full flex flex-col justify-end text-primary px-[16px]">
                    <div>
                        <ButtonRounded icon={<SidebarIcon />} onClick={() => setIsMinize(!isMinimize)} />
                    </div>
                </section>
            </motion.nav>
        </div>
